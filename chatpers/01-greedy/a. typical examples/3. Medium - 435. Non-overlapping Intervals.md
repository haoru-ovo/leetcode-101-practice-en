...
## Stable Sort Behavior — Summary / 稳定排序行为总结

Given intervals with equal end values, for example:
...
### Explanation
- Python uses a **stable sorting algorithm**.
- Stable means: if two intervals have the **same sorting key**, their **original order is preserved**.
- Here, both [2,3] and [1,3] end at 3.
- Since [2,3] appears earlier in the original list, it stays before [1,3] after sorting.

### 解释
- Python 的排序是 **稳定排序**。
- 所谓稳定：当两个区间的排序 key **相同** 时，它们在排序后的**相对顺序保持不变**。
- 在例子中，[2,3] 和 [1,3] 的 end 都是 3。
- 因为 [2,3] 在原列表中排在前面，所以排序后仍然排在 [1,3] 的前面。

### Final Summary / 最终小结
- English: Same end → keep original order.
- 中文：结束相同 → 按原顺序排列。
