...
class Solution:
    def findContentChildren(self, g: List[int], s: List[int]) -> int:
        g.sort()  # set of children greed factor
        s.sort()  # set of cookies number
        i = j = 0   # i child，j cookie
        len_children, len_cookies = len(g), len(s)

        while i < len_children and j < len_cookies:
            if s[j] >= g[i]:         # This cookie will satisfy the child
                i += 1               # feed one child
            j += 1                   # Use this cookie (always move)
        return i
